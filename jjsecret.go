...
func main() {
	...
	// Load url_map.txt for directory scans (maps filename → original URL)
	// Generated by recon-js.sh; lets local scan results show the live URL.
	urlMap := make(map[string]string)
	if dirFlag != "" {
		if data, err := os.ReadFile(filepath.Join(dirFlag, "url_map.txt")); err == nil {
			...
			if len(urlMap) > 0 && !silentFlag {
				fmt.Printf("[*] url_map.txt loaded: %d URL mappings\n", len(urlMap))
			}
		}
	}

	// displayTarget resolves a local file path to its original URL via url_map.txt.
	// Falls back to the original target if no mapping is found.
	displayTarget := func(target string) string {
		if len(urlMap) == 0 {
			return target
		}
		if mapped, ok := urlMap[filepath.Base(target)]; ok {
			return mapped
		}
		return target
	}
	...
	go func() {
		...
		for res := range results {
			...
			if jsonFlag != "" || sarifFlag != "" || mdFlag != "" {
				res.Target = displayTarget(res.Target)
				resultsMu.Lock()
			...
			fmt.Printf("\033[36m[%s]\033[0m %s[%s]\033[0m \033[32m%s\033[0m : %s\n", displayTarget(res.Target), consoleColor, res.Priority, res.Name, res.Match)

			// File Output
			if f != nil {
				fmt.Fprintf(f, "[%s] [%s] %s : %s\n", displayTarget(res.Target), res.Priority, res.Name, res.Match)
			}
			...
			if csvWriter != nil {
				...
				csvWriter.Write([]string{displayTarget(res.Target), res.Priority, res.Name, res.Match, strconv.Itoa(res.Confidence), res.Provider, lineStr})
			}
		}
	}()
